...
export default function Page() {
    ...
    const getOrder = async () => {
        const response = await fetch(`http://localhost:8080/api/order/seller/${localStorage.getItem("token")}/${orderid}/${productid}`)
    ...
    const makeTransit = async () => {
        const response = await fetch(
            `http://localhost:8080/api/order/transit/${localStorage.getItem("token")}/${orderid}/${productid}`,
            {
                method: "PUT"
            }
        )
                                    ...
                                    <div className="flex-1">
                                        <div className="flex h-full flex-col gap-3">
                                            ...
                                            <div className="w-full h-12">
                                                <button disabled={productDetails.status == "in transit"} onClick={() => { makeTransit() }} className={`${productDetails.status == "in transit" ? "bg-gray-100 text-gray-600 border-gray-600" : "bg-green-50 border-green-600 text-green-600"} border w-full h-full `}>
                                                    Ship Order
                                                </button>
                                            </div>
